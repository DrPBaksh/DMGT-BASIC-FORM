...
import s3UploadService, { validateFileType, validateFileSize, formatFileSize } from '../services/s3UploadServiceFixed';
...
const FormRenderer = ({ 
  ...
  // FIXED: Enhanced file handling with proper S3 integration
  const handleInputChange = async (questionId, value, file = null) => {
    ...
    // FIXED: Enhanced file upload handling with S3 integration
    if (file) {
      try {
        ...
        if (s3UploadService.isServiceConfigured()) {
          try {
            console.log('Using S3 upload service');
          ...
          <div className="file-upload-container">
            ...
            <div className="file-upload-text">
              ...
              <small className="upload-note">
                📁 Files are uploaded to AWS S3 with proper organization and metadata tracking.
              </small>
            </div>
            ...
            {/* Enhanced file display with S3 support */}
              ...
              <div className="uploaded-file">
                <div className="file-info">
                  ...
                  <span className="file-note">
                    {fileInfo.storedInS3 ? '(Stored in S3)' : '(Previously uploaded)'}
                  </span>
                  {fileInfo.uploadId && (
                    <span className="file-id">ID: {fileInfo.uploadId}</span>
                  )}
                </div>
              </div>
            )}
  ...
  const renderInputField = (question, value) => {
    ...
    const cleanValue = value ? 
      value.replace(/\s*\[FILE_UPLOADED:.*?\]\s*/, '')
           .replace(/\s*\[FILE_ATTACHED:.*?\]\s*/, '')
           .replace(/\s*\[FILE:.*?\]\s*/, '')
           .trim() : '';
          ...
          <div className="completion-note">
            {answeredQuestions.size === questions.length ? (
              ...
              <div className="completion-encouragement">
                Keep going! You're making great progress. Files are automatically saved to S3.
              </div>
            )}
          </div>
        </div>
