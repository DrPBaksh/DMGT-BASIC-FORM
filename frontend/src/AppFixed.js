...
import FormRendererFixed from './components/FormRendererFixed';
...
function App() {
  ...
  // Enhanced Company Form State Management
  ...
  // FIXED: Enhanced session management for both tabs
  useEffect(() => {
    if (activeTab === 'company') {
      ...
      // For employee tab, reset session state if not initialized
  ...
  // FIXED: Enhanced question loading with proper dependency management
  useEffect(() => {
  ...
  // FIXED: Enhanced reset and initialization when company ID changes
  useEffect(() => {
    if (companyId) {
  ...
  // FIXED: Enhanced company status checking with proper logic
  const checkCompanyStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // FIXED: Proper form state determination - only one company questionnaire per company
        const hasAnyResponses = status.completionPercentage > 0;
        const isExplicitlyComplete = status.explicitlyCompleted === true;
        
        if (isExplicitlyComplete) {
          setCompanyFormState('completed');
          console.log('Company form is explicitly completed');
          ...
          setCompanyFormState('new');
          console.log('New company form - ready for input');
        }
  ...
  // FIXED: Enhanced loading of existing company responses
  ...
  // FIXED: Enhanced saveResponse function with proper company logic
  const saveResponse = async (questionId, answer, file = null) => {
    ...
    // FIXED: Better session validation for employee forms
    ...
    try {
      ...
      // FIXED: Proper employee session handling with ID notification
      if (activeTab === 'employee') {
        ...
        // FIXED: Company form handling - allow modifications, only one per company
      ...
      // FIXED: Handle company status updates properly
      if (activeTab === 'company' && responseData) {
        if (responseData.completionPercentage !== undefined) {
          setCompanyStatus(prev => ({
            ...
            companyInProgress: responseData.inProgress,
            companyCompleted: responseData.explicitlyCompleted === true,
            lastModified: new Date().toISOString()
          }));
        }
        
        if (responseData.explicitlyCompleted === true) {
          setCompanyFormState('completed');
      ...
      // FIXED: Enhanced employee ID capture with user notification
      if (activeTab === 'employee' && 
        ...
        // FIXED: Notify user of their assigned employee ID
        alert(`Your Employee ID has been assigned: #${responseData.employeeId}\n\nPlease save this ID if you want to return to your assessment later!`);
  ...
  // FIXED: Enhanced tab change handling
  const handleTabChange = (tabId) => {
    ...
    // FIXED: Better state management when switching tabs
    if (tabId === 'employee') {
      // Clear responses when switching to employee tab unless session exists
      if (!sessionInitialized) {
        setResponses({});
        setQuestions([]);
        resetEmployeeSession();
  ...
  // FIXED: Enhanced employee session setup with proper notification
  const handleEmployeeSessionSetup = async (mode, employeeId = null) => {
    ...
    if (mode === 'new') {
      ...
      // FIXED: Notify user about employee ID assignment
      alert('New employee session started!\n\nYour Employee ID will be assigned after your first response is saved. Please remember this ID if you want to return to your assessment later.');
  ...
  // FIXED: Enhanced company status indicator
    ...
    return (
      <div className={`company-status-indicator ${statusInfo.class}`}>
        ...
        <div className="modification-notice">
          Only one company questionnaire per Company ID. Responses are auto-saved after each question.
        </div>
      </div>
    );
  };
