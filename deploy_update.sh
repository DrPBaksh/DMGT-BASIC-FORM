...
#####################################
# DMGT Basic Form - Enhanced Deploy/Update Script
# Deploys/updates infrastructure and frontend for Data & AI Readiness Assessment
# Optimized for EU-West-2 region with smart update logic
#####################################
...
# Configuration with defaults - CHANGED to eu-west-2
MODE="deploy"
ENVIRONMENT=${ENVIRONMENT:-dev}
REGION=${AWS_REGION:-eu-west-2}  # Changed from us-east-1
...
show_usage() {
    echo -e "${BOLD}DMGT Basic Form Deploy/Update Script${NC}"
    echo ""
    ...
    echo -e "  --skip-backend    : Skip backend deployment (update mode)"
    ...
    echo -e "  ${CYAN}$0${NC}                                    # Deploy/update to dev"
    echo -e "  ${CYAN}$0 --environment=prod --verbose${NC}       # Deploy to prod with verbose logging"
    echo -e "  ${CYAN}$0 --frontend-only${NC}                   # Update frontend only"
    echo -e "  ${CYAN}$0 --skip-backend${NC}                    # Update mode (skip backend)"
...
check_prerequisites() {
    ...
    if [[ "$MODE" != "delete" ]]; then
        ...
        if [ "$node_major" -ge 18 ]; then
            log_warning "Node.js v${node_major} detected. Setting legacy OpenSSL provider for compatibility."
    ...
    if [[ "$MODE" != "delete" && "$SKIP_BACKEND" != "true" ]]; then
...
check_backend_status() {
    ...
    if aws cloudformation describe-stacks --stack-name $STACK_NAME --region $REGION > /dev/null 2>&1; then
        ...
        log_success "Existing stack found: $STACK_NAME (Status: $stack_status)"
        ...
        log_info "Testing backend API functionality..."
        if test_backend_apis; then
            BACKEND_WORKING=true
            log_success "Backend APIs are working correctly"
            if [ "$SKIP_BACKEND" != "true" ]; then
                log_info "Backend is working. Use --skip-backend for frontend-only updates"
            fi
        else
            log_warning "Backend APIs not responding correctly - backend update needed"
            BACKEND_WORKING=false
        fi
    else
        log_info "No existing stack found - full deployment needed"
        STACK_EXISTS=false
        if [ "$SKIP_BACKEND" = "true" ]; then
            log_error "Cannot skip backend - no existing infrastructure found"
            exit 1
        fi
    fi
}
...
test_backend_apis() {
    ...
    if [ -z "$api_url" ] || [ "$api_url" = "None" ]; then
        log_debug "Could not retrieve API URL from stack outputs"
        return 1
    fi
...
deploy_infrastructure() {
    if [ "$SKIP_BACKEND" = "true" ]; then
        log_info "Skipping backend deployment as requested"
        return 0
    fi
    ...
    if [ "$STACK_EXISTS" = "true" ]; then
        ...
        log_info "Creating UPDATE changeset for existing stack"
    else
        log_info "Creating CREATE changeset for new stack"
    fi
    ...
    if [ "$changeset_status" = "FAILED" ]; then
        local status_reason=$(aws cloudformation describe-change-set \
            --stack-name $STACK_NAME \
            --change-set-name $changeset_name \
            --region $REGION \
        ...
        if [[ "$status_reason" == *"didn't contain changes"* ]]; then
            log_success "No infrastructure changes needed - stack is up to date"
            aws cloudformation delete-change-set --stack-name $STACK_NAME --change-set-name $changeset_name --region $REGION > /dev/null 2>&1
            local cf_end_time=$(date +%s)
            local cf_duration=$((cf_end_time - cf_start_time))
            log_success "Infrastructure check completed in ${cf_duration}s"
            return 0
        else
            log_error "Changeset creation failed: $status_reason"
            exit 1
        fi
    fi
...
upload_configuration_files() {
    if [ "$SKIP_BACKEND" = "true" ]; then
        log_info "Skipping configuration file upload (backend skipped)"
        return 0
    fi
...
build_frontend() {
    ...
    execute_command "npm install" "NPM dependencies installation"
...
deployment_summary() {
    ...
    echo -e "Mode: ${CYAN}$([ "$SKIP_BACKEND" = "true" ] && echo "Frontend-only update" || echo "Full deployment")${NC}"
    echo ""
...
main() {
    ...
    printf '=%.0s' {1..80}
    echo ""
    echo "  DMGT Basic Form - Enhanced Deploy/Update Script"
    echo "  Optimized for EU-West-2 with Smart Update Logic"
    printf '=%.0s' {1..80}
    echo -e "${NC}"
    ...
    case $MODE in
        "delete")
            ...
            # destroy_infrastructure  # Not implementing delete in this version
            echo "Delete functionality not implemented in this version"
            ;;
        "frontend-only")
            TOTAL_STEPS=4
            SKIP_BACKEND=true
